...
public class UserValidator implements Validator {
    ...
    public void validate(Object object, Errors errors){
        ...
        Optional<User> user=Optional.ofNullable(userRepository.findByUsername(userRequestDto.getUsername()).get());
        ...
        if(userRepository.existsByEmail(email)){    // 이메일 중복
            if((user != null && !email.equals(user.get().getEmail())) || user == null) {    // 수정한 이메일 또는 가입할 이메일
        ...
        if(userRepository.existsByPhone(phone)){  // 연략처 중복
            if((user != null && !phone.equals(user.get().getPhone())) || user == null) {    // 수정한 연락처 또는 가입할 연락처
